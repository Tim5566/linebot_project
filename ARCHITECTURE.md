...
- **AdSense 審核歷史**：第一次送審時（網站剛架好不久）被判定「低品質/低價值內容」，
  ...
  再次送審。**第二次送審（2026/09/02）同樣被判定「缺乏價值的內容」**，這次診斷出
  首頁與多個工具頁靜態文字過少（多在 650～1000 字，內容主要靠 JS 動態載入）、
  20 篇教學文雖紮實卻只藏在 drawer 選單、首頁主內容流沒有露出。**2026/09/05 已補內容**：
  首頁補「精選教學文章」卡片區（連到 6 篇教學文）+ 開場說明段落；`twse_top100` /
  `otc_top100` / `fund_flow` / `ma_finder` / `sector_heatmap` 這 5 個工具頁底部各補一段
  ~800 字的解說卡片（沿用 `notice.html`/`disposal.html` 已驗證過的 `.doc-card` 樣式，
  不新增 CSS、不動工具本體）。`news.html` 檢查後發現本來就已有完整解說卡片，未變動。
  補完後尚未送複查，計畫先在 GSC 對這幾個頁面要求重新建立索引、確認新內容已被收錄
  後再送。
