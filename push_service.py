...
def start_scheduler(line_bot_api):
    ...
    # 三大法人個股買賣超更新時間
    scheduler.add_job(lambda: broadcast_post_inf(line_bot_api, 3), 'cron', hour=15, minute=0, timezone=taiwan) #15:00
    scheduler.add_job(lambda: broadcast_post_inf(line_bot_api, 4), 'cron', hour=16, minute=10, timezone=taiwan) #16:10
    scheduler.add_job(lambda: broadcast_post_inf(line_bot_api, 5), 'cron', hour=16, minute=10, timezone=taiwan) #16:10

    ##大盤融資卷總金額統計
    scheduler.add_job(lambda: broadcast_post_inf(line_bot_api, 6), 'cron', hour=21, minute=0, timezone=taiwan) #21:00

    # 個股借卷賣出
    scheduler.add_job(lambda: broadcast_post_inf(line_bot_api, 7), 'cron', hour=21, minute=30, timezone=taiwan) #21:30
