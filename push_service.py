...
def broadcast_post_inf(line_bot_api, label):
    ...
    if label == 1:
        ...
        message = TextSendMessage(text="📢 今盤後，大盤融資金額已更新❗")
        market_pnfo()
    elif label == 6:
...
def start_scheduler(line_bot_api):
    ...
    # 盤後整體資訊更新時間
    scheduler.add_job(lambda: broadcast_post_inf(line_bot_api, 1), 'cron', hour=15, minute=40, timezone=taiwan) #15:00
    ...
    ##大盤融資卷總金額統計
    scheduler.add_job(lambda: broadcast_post_inf(line_bot_api, 5), 'cron', hour=20, minute=30, timezone=taiwan) #20:30
